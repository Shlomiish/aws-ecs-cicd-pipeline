...
describe('server api', () => {
  it('GET /api/health returns ok:true', async () => {
    const app = createApp({ sendLogToSQS: async () => {} });
    const res = await request(app).get('/api/health').expect(200);
    expect(res.body).toEqual({ ok: true });
  });

  it('GET /api/button1 returns payload and calls SQS logger', async () => {
    const sendLogToSQS = vi.fn(async () => {});
    ...
    expect(res.body.message).toBe('Hello from API #1');
    expect(typeof res.body.timestamp).toBe('string');
    expect(sendLogToSQS).toHaveBeenCalledTimes(1);
    expect(sendLogToSQS.mock.calls[0][0].button).toBe('button1');
  });

  it('if SQS logger fails, endpoint returns 500', async () => {
    const sendLogToSQS = vi.fn(async () => {
